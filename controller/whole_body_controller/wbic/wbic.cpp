                             ...
                             Eigen::VectorXd &wbc_qddot_cmd)
{
    ...
    for (auto it = task_vector.begin(); it != task_vector.end(); ++it)
    {
        if (it == task_vector.begin())
        {
            ...
            delta_q_cmd = JtPre_pinv * first_task->KpIK().cwiseProduct(first_task->PosError()); // global err
            qdot_cmd = JtPre_pinv * first_task->DesiredVel();
            qddot_cmd = qddot_cmd + JtPre_bar * (first_task->OpCommand() - JtDotQdot - Jt * qddot_cmd);
        ...
        else
        {
            ...
            delta_q_cmd = prev_delta_q_cmd
                        + JtPre_pinv * (task->KpIK().cwiseProduct(task->PosError()) - Jt * prev_delta_q_cmd);
            qdot_cmd = prev_qdot_cmd
                        + JtPre_pinv * (task->DesiredVel() - Jt * prev_qdot_cmd);
            qddot_cmd = prev_qddot_cmd
                        + JtPre_bar * (task->OpCommand() - JtDotQdot - Jt * prev_qddot_cmd);
            // Eigen::JacobiSVD<Eigen::MatrixXd> svd(JtPre, Eigen::ComputeThinU |
            // Eigen::ComputeThinV);
            // util::PrettyPrint(svd.singularValues(), std::cout, "singular values");
        }
 ...
 * @param contact_map
 */
void WBIC::_BuildContactMtxVect(const std::map<std::string, Contact *> &contact_map)
{
 ...
 * @param force_task_vector
 */
void WBIC::_GetDesiredReactionForce(const std::vector<ForceTask *> &force_task_vector)
{
    for (auto it = force_task_vector.begin(); it != force_task_vector.end(); ++it)
    {
...
void WBIC::_SetQPCost(const Eigen::VectorXd &wbc_qddot_cmd)
{
    ...
    Eigen::MatrixXd delta_qddot_cost = (wbic_data_->qp_params_->W_delta_qddot_).asDiagonal();
    Eigen::MatrixXd xc_ddot_cost = (Jc_.transpose() * (wbic_data_->qp_params_->W_xc_ddot_).asDiagonal() * Jc_).topLeftCorner(num_floating_, num_floating_);
    H_.topLeftCorner(num_floating_, num_floating_) = delta_qddot_cost + xc_ddot_cost;
    H_.bottomRightCorner(dim_contact_, dim_contact_) = (wbic_data_->qp_params_->W_delta_rf_).asDiagonal();
    g_ = Eigen::VectorXd::Zero(num_floating_ + dim_contact_);

    g_.head(num_floating_) = (wbc_qddot_cmd.transpose()
                                * Jc_.transpose()
                                * (wbic_data_->qp_params_->W_xc_ddot_).asDiagonal() * Jc_+ JcDotQdot_.transpose()
                                * (wbic_data_->qp_params_->W_xc_ddot_).asDiagonal() * Jc_).head(num_floating_);
}
...
void WBIC::_SolveQP(const Eigen::VectorXd &wbc_qddot_cmd)
{
    ...
    wbic_data_->corrected_wbc_qddot_cmd_.head(num_floating_) += wbic_data_->delta_qddot_;
    wbic_data_->rf_cmd_ = des_rf_ + wbic_data_->delta_rf_;
    wbic_data_->Xc_ddot_ = Jc_ * wbic_data_->corrected_wbc_qddot_cmd_
                            + JcDotQdot_; // contact constraint check (this should be almost zero)

    wbic_data_->delta_qddot_cost_ = wbic_data_->delta_qddot_.transpose()
                                    * (wbic_data_->qp_params_->W_delta_qddot_).asDiagonal()
                                    * wbic_data_->delta_qddot_;
    wbic_data_->delta_rf_cost_ = wbic_data_->delta_rf_.transpose()
                                * (wbic_data_->qp_params_->W_delta_rf_).asDiagonal()
                                * wbic_data_->delta_rf_;
    wbic_data_->Xc_ddot_cost_ = wbic_data_->Xc_ddot_.transpose()
...
{
    Eigen::VectorXd trq_trc = M_.bottomRows(num_qdot_ - num_floating_)
    ...
    Eigen::MatrixXd UNi_trc = (sa_ * Ni_dyn_).rightCols(num_qdot_ - num_floating_);
    ...
    jtrq_cmd = sa_.rightCols(num_qdot_ - num_floating_).transpose() * jtrq_cmd; // dimension: num_active_ + num_passive_
